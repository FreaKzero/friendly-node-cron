...
const dayParser = (pattern, string) => {
  const daymatch = string.match(/(\d+)? (sun|mon|tue|wed|thu|fri|sat|weekends|workdays)/g)
  ...
  if (daymatch) {
    ...
    if (day === "workdays") {
      ...
      const days = daymatch.map(i => ["","mon","tue","wed","thu","fri","sat", "sun"].indexOf(i.trim()))
      if (days.length) {
...
const dateParser = (pattern, string) => {
  const full = /(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec) ([0-9]{1,2})/.exec(string);
